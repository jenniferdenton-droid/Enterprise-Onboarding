...
export default async function handler(req, res) {
  // Lenient: only validate IF key is sent. Vercel Authentication gates the URL.
  ...
  try {
    ...
    if (req.method === "DELETE") {
      ...
      const channelId = req.query?.slack_channel_id;
      if (!id) return res.status(400).json({ error: "hs_object_id required" });
      if (channelId) {
        // Remove just one channel from the mapping
        await removeChannelFromMapping(id, channelId);
      } else {
        // Remove the entire mapping
...
async function postWelcome(token, channelId, companyName) {
  try {
    await fetch("https://slack.com/api/chat.postMessage", {
      ...
      body: JSON.stringify({
        channel: channelId,
        text: `:rocket: Channel created for *${companyName}* onboarding. The Enterprise Onboarding bot is now listening — blockers, risks, and progress notes will flow into the dashboard.`
      })
    });
  } catch { /* non-fatal */ }
}
