...
Return ONLY a JSON object (no markdown, no explanation, no preamble) keyed by the same account keys as the input. For each account, return up to 15 messages spanning the full 14-business-day window, newest first. Include ALL blockers, ALL risks, and the most informative updates — don't artificially cap if there are more substantive items. Prioritize blockers > risks > updates when ordering. Combine highly-related back-and-forth into a single entry where possible.
...
${JSON.stringify(promptInput, null, 2)}`;
  ...
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 12000,  // higher cap to fit 15 categorized items × 37 accounts
