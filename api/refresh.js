...
async function categorizeMessages(messagesByCompany, apiKey) {
  ...
  const promptFor = (chunkInput) => `You are categorizing Slack messages from Moxie's enterprise medspa onboarding channels. Each account has its own channel; all messages below are about the named account.
...
IMPORTANT: Return items SPANNING THE FULL 14-business-day window. Do NOT only return the newest 2-3 days. Include items from earlier in the window too (e.g., 8, 10, 12 days ago).

Return ONLY a JSON object (no markdown) keyed by account name. For each account, return up to 15 messages, newest first. Include ALL blockers, ALL risks, plus the most informative updates. Prioritize blockers > risks > updates when ordering.

{
  "rivkin": [
    {"type":"blocker","date":"May 21","text":"<concise summary>"},
    {"type":"update","date":"May 16","text":"<concise summary>"},
    ...
  ],
  ...
}
